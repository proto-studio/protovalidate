...
// TestDomainRuleSet_NoConflict_WithNilParent tests:
//   - noConflict correctly handles the case where a ruleset with parent == nil conflicts
//   - This tests the edge case where someone directly creates a DomainRuleSet (not using factory)
//     with a conflict type set, then calls a conflicting method
func TestDomainRuleSet_NoConflict_WithNilParent(t *testing.T) {
	// Create a ruleset directly (not using factory) with parent == nil and conflictType set
	directRuleSet := &DomainRuleSet{
		parent:       nil,
		conflictType: domainConflictTypeRequired,
	}
