...
// TestStringMinRule_String_Truncation tests that the rule String() method truncates long strings.
func TestStringMinRule_String_Truncation(t *testing.T) {
	// Create a very long string (longer than 50 characters)
	longString := "a"
	for i := 0; i < 100; i++ {
		longString += "b"
	...
	ruleSet := rules.String().WithMin(longString)
	...
	// With prefix "StringRuleSet.WithMin(\"" and suffix "...\")", total should be < 120
	if len(ruleStr) > 120 {
